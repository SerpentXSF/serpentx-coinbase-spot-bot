...
def score_product(p):
    ...
    if len(closes)<30 or len(tcloses)<20 or px<=0:
        return {**base(p), 'score':0, 'action':'SKIP', 'reasons':['insufficient candle history'], 'cautions':cautions}
    ...
    if p.get('trading_disabled'):
        score-=5; cautions.append('trading disabled')
    action='BUY_WATCHLIST' if score>=5 else ('WATCH' if score>=3 else 'HOLD_USDC')
    return {**base(p), 'score':score, 'action':action, 'price':px, 'change_24h':chg, 'volume_24h':vol24, 'quote_volume_24h_usdc': quote_vol, 'rsi':rrsi, 'ema9':e9, 'ema21':e21, 'ema50':e50, 'trend_ema20':t20, 'trend_ema50':t50, 'avg_15m_range_pct':avg_range*100, 'reasons':reasons, 'cautions':cautions, 'quote_min_size': quote_min}
...
def main():
    ...
    for p in ps:
        pid=p.get('product_id','')
        if pid.endswith('-USDC') and not p.get('trading_disabled'):
            usdc.append(p)
    rows=[]
    ...
    out={'generated_at':datetime.now(timezone.utc).isoformat(), 'total_products_seen':len(ps), 'usdc_pairs_analyzed':len(usdc), 'rows':rows, 'top5':rows[:5]}
    ...
    print(json.dumps({'generated_at':out['generated_at'], 'total_products_seen':len(ps), 'usdc_pairs_analyzed':len(usdc), 'top5':[{k:r[k] for k in ['product_id','score','action','price','change_24h','quote_volume_24h_usdc','rsi','avg_15m_range_pct','reasons','cautions'] if k in r} for r in rows[:5]], 'latest_path':str(latest)}, indent=2))
